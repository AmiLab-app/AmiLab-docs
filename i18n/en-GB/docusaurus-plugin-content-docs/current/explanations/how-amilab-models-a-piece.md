...
- its **type** — `dc`, `htr`, `tr`, `dtr`, or `ch` (see the [stitch types reference](../reference/stitch-types.md))
...
An **increase** is the mirror image, and AmiLab does not need a special record for it. When two stitches in a round share the same base — two stitches worked into one loop — that is an increase. AmiLab recognises it from the shape of the graph.
...
- [Tension and dimensions](./gauge-and-dimensions.md) — how stitch counts become real-world measurements
