...
description: How a profile's silhouette and cross-section combine into a 3D shape you can materialize.
---
...
- The **height** of a point is its position from the bottom of the piece, in centimeters.
- The **radius** of a point is how far the shape reaches out from its center axis at that height. AmiLab stores radius as a fraction from 0 (on the axis) to 1 (the profile's maximum radius), so scaling the whole shape is a single setting.
...
A profile is a reusable *template*, not a pattern by itself. When you [materialize](./materializing-shapes.md) it, AmiLab combines the silhouette and cross-section with your [gauge](./gauge-and-dimensions.md) to work out the stitch count for each round, then generates the stitches — exactly as it does for the built-in shapes. The profile stays saved, ready to materialize again at any size or gauge.

## Related

- [Materializing shapes](./materializing-shapes.md) — how a profile becomes stitches
- [Profile editor controls](../reference/profile-editor-controls.md) — the controls in the Shapes view
- [Create a custom shape](../tutorials/create-a-custom-shape.md) — design and materialize a profile step by step
